...
do 
    for d in $d_f
    do
        bsub -M 46000 -R "rusage[mem=46000]" -P methCNV -e AllCNVs.lsferr -o AllCNVs.lsfout "bash PrepareROCInput_ClusterWrapper.sh 'mean(res[,5])+sd(res[,5])' 'mean(res[,5])-sd(res[,5])'"
    done
done
```
